...
4. **Dual host support.** The tool must work under both classic MSBuild.exe (.NET Framework projects) and the SDK/`dotnet build` host (.NET Core), which have different bundled `Microsoft.Build.*` assembly versions.
...
| Dual host support | OK. XML targets are host-agnostic by nature. |
...
| Dual host support | Requires care. Custom `Task` assemblies must not ship their own copy of `Microsoft.Build.dll`; they build against the API surface and let the hosting MSBuild's assemblies bind at runtime. This means multi-targeting the task assembly (e.g. `net472` for MSBuild.exe, `net8.0` for the SDK) and laying out the NuGet package accordingly. |
...
- The library is distributed as a NuGet package that multi-targets the task assembly (e.g. `net472` and `net8.0`) so it works correctly under both classic MSBuild.exe and the modern SDK/`dotnet build` hosts, following the standard NuGet `tasks/<tfm>/*.dll` + `build/*.props`/`*.targets` convention for MSBuild task packages.
